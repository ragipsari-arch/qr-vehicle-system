...
import { sendNotification, getSimulatorLogs, clearSimulatorLogs } from '../services/notification';

const router = Router();

// Store interactive math captcha challenge keys per session / IP
// Simple local in-memory store for local ease of use
const activeCaptchas: Record<string, number> = {};

// Get public vehicle info
router.get('/vehicles/:qr_uuid', async (req: Request, res: Response) => {
  try {
    const { qr_uuid } = req.params;

    const vehicleResult = await query('SELECT * FROM vehicles WHERE qr_uuid = $1', [qr_uuid]);
    ...
    const ownerResult = await query('SELECT plan FROM users WHERE id = $1', [ownerId]);
    const ownerPlan = ownerResult.rows[0]?.plan || 'free';

    const settingsResult = await query('SELECT * FROM notification_settings WHERE user_id = $1', [ownerId]);
...
router.post('/vehicles/:qr_uuid/notify', async (req: Request, res: Response) => {
  try {
    const { qr_uuid } = req.params;
    ...
    const ownerResult = await query('SELECT email, phone, plan FROM users WHERE id = $1', [ownerId]);
    ...
    const count = parseInt((rateLimitCheck.rows[0] as any).count || '0');
    if (count >= 3) {
    ...
    await query(
      `INSERT INTO messages (id, vehicle_id, category, sub_category, custom_text, location_lat, location_lng, sender_ip, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [messageId, vehicle.id, category, sub_category || null, custom_text || null, lat, lng, ip, 'pending']
    );
    ...
    await query('UPDATE messages SET status = $1 WHERE id = $2', [finalStatus, messageId]);
...
router.get('/simulator/logs', (req: Request, res: Response) => {
  res.json(getSimulatorLogs());
});
