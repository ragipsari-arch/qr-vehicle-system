...
export const dbEngine = hasPgUrl ? 'postgres' : 'jsonfile';

console.log(`[DB] Using Database Engine: ${dbEngine.toUpperCase()}`);
...
const jsonDbPath = path.resolve(__dirname, '../../qr_vehicle_system_db.json');
let localDbData: JsonDbSchema = {
  users: [],
  vehicles: [],
...
function saveJsonDb() {
  try {
    fs.writeFileSync(jsonDbPath, JSON.stringify(localDbData, null, 2), 'utf8');
  } catch (err: any) {
...
export async function query<T = any>(sql: string, params: any[] = []): Promise<{ rows: T[] }> {
  if (dbEngine === 'postgres') {
    if (!pgPool) throw new Error('[DB] Postgres Pool not initialized');
  ...
  if (normalizedSql.startsWith('INSERT INTO notification_settings')) {
    ...
    saveJsonDb();
    return { rows: [newSettings as any] };
  }
  ...
  if (normalizedSql.startsWith('SELECT COUNT(*) as count FROM vehicles WHERE user_id = $1')) {
    ...
    return { rows: [{ count: count.toString() }] as any };
  }
  ...
  if (normalizedSql.startsWith('SELECT COUNT(*) as count FROM users')) {
    return { rows: [{ count: localDbData.users.length.toString() }] as any };
  }
  ...
  if (normalizedSql.startsWith('SELECT COUNT(*) as count FROM vehicles')) {
    return { rows: [{ count: localDbData.vehicles.length.toString() }] as any };
  }
  if (normalizedSql.startsWith('SELECT COUNT(*) as count FROM messages')) {
    return { rows: [{ count: localDbData.messages.length.toString() }] as any };
  }
...
export async function initDb() {
  ...
  if (checkAdmin.rows.length === 0) {
    ...
    await query(
      ...
      [adminId, adminEmail, '+905555555555', hash, 'admin', 'premium', 1, 1]
    );
    ...
    await query(
      ...
      ['settings-admin-uuid', adminId, 1, 1, 1, 1, 1]
    );
