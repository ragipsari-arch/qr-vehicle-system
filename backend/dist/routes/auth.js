...
router.post('/register', async (req, res) => {
    try {
        ...
        // Create User
        await (0, db_1.query)(`INSERT INTO users (id, email, phone, password_hash, role, plan, is_phone_verified, is_email_verified)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [userId, email, phone, passwordHash, 'user', 'free', 0, 0]);
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [settingsId, userId, 0, 0, 0, 0, 1]);
        ...
        await (0, db_1.query)(`INSERT INTO verification_codes (id, user_id, type, code, expires_at, is_used)
       VALUES ($1, $2, $3, $4, $5, $6)`, [otpId, userId, 'phone_otp', otpCode, expiresAt, 0]);
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`SELECT * FROM verification_codes 
       WHERE user_id = $1 AND code = $2 AND is_used = 0`, [userId, code]);
        ...
        // Mark code as used
        await (0, db_1.query)('UPDATE verification_codes SET is_used = 1 WHERE id = $1', [otpRecord.id]);
        // Verify user phone
        await (0, db_1.query)('UPDATE users SET is_phone_verified = 1 WHERE id = $1', [userId]);
...
router.post('/resend-otp', async (req, res) => {
    try {
        ...
        await (0, db_1.query)('UPDATE verification_codes SET is_used = 1 WHERE user_id = $1 AND type = $2', [userId, 'phone_otp']);
        // Create new OTP code
        await (0, db_1.query)(`INSERT INTO verification_codes (id, user_id, type, code, expires_at, is_used)
       VALUES ($1, $2, $3, $4, $5, $6)`, [otpId, userId, 'phone_otp', otpCode, expiresAt, 0]);
        console.log(`\n======================================================`);
...
router.post('/login', async (req, res) => {
    try {
        ...
        const userResult = await (0, db_1.query)('SELECT * FROM users WHERE email = $1 OR phone = $2', [emailOrPhone, emailOrPhone]);
        if (userResult.rows.length === 0) {
